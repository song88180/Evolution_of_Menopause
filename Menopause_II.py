...
x,y = get_moltality_curve(max_age=max_age)
...
for year in range(N_years + 1):
    ...
    if if_lifespan:
        ...
        _age_, _moltality_ = get_moltality_curve(max_age)
        Primary_mortality_with_age_female = dict(zip(_age_, _moltality_))
        Primary_mortality_with_age_male = Primary_mortality_with_age_female
    ...
    for key in allele_list_dict.keys():
        if key in allele_dict:
            allele_list_dict[key].append(allele_dict[key])
        else:
